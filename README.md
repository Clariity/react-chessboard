...
#### IMPORTANT: Examples use the current stable release of chess.js. As of writing this, chess.js v1.0.0 is still in beta. These examples use chess.js ^0.13.4
...
export const RandomMoveEngine = () => {
  ...
  function makeBoardMove(
    ...
    const moveConfig: MoveConfig = { from, to, promotion };
    return game.current.move(moveConfig);
  }
  ...
  function makeRandomMove() {
    try {
      ...
      makeBoardMove(randomMove.from, randomMove.to);
      updateBoard();
    } catch (error) {
      console.error("Error while RandomMove", error);
      return;
    }
  }
  ...
  function handlePieceDrop(sourceSquare: Square, targetSquare: Square) {
    try {
      console.log(`Move detected: ${sourceSquare} to ${targetSquare}`);
      const move = makeBoardMove(sourceSquare, targetSquare);
      ...
      console.error("Error while Moving piece", error);
      return false;
    }
  }
